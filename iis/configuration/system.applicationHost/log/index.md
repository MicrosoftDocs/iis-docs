...
| --- | --- |
| `centralLogFileMode` | Optional enum attribute.<br><br>Specifies the central logging mode for the server.<br><br>The **centralLogFileMode** attribute can have one of the following possible values. The default is `Site`. <table> <tbody> <tr> <td>Value</td> <td>Description</td></tr> <tr> <td><code>Site</code></td> <td>Configures all sites to use site logging instead of central logging. This is the default setting.<br><br>The numerical value is <strong>0</strong>.</td></tr> <tr> <td><code>CentralBinary</code></td> <td>Creates one log file for all Web sites on a Web server. The data in the log file is binary-based, unformatted data that is not customizable.<br><br>The numerical value is <strong>1</strong>.</td></tr> <tr> <td><code>CentralW3C</code></td> <td>Logs requests for all sites on a Web server to a single central log file in a text-based, customizable ASCII format.<br><br>The numerical value is <strong>2</strong>.</td></tr></tbody></table> |
