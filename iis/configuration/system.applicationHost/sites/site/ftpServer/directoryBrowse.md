...
| --- | --- |
| `showFlags` | Optional flags attribute. Specifies the options for directory listings. | Name | Value | | --- | --- | | `LongDate` | Specifies whether to show long dates or short dates. The numeric value is `2`. | | `StyleUnix` | Specifies whether to display UNIX-style directory listings; otherwise, displays MSDOS-style listings. The numeric value is `4`. | | `DisplayAvailableBytes` | Specifies whether to display the available bytes in directory listings. The numeric value is `16`. | | `DisplayVirtualDirectories` | Specifies whether to display virtual directories if set; otherwise, virtual directories are hidden. The numeric value is `32`. | | `UseGmtTime` | Specifies whether to display dates and times in GMT. The numeric value is `64`. | There is no default value. |
