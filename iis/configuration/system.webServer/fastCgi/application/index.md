...
| `monitorChangesTo` | Option string attribute. Specifies the full path to a file to monitor for changes. When changes to the file are detected, IIS will terminate and restart the FastCGI application that is specified in the `fullPath` attribute. **Note:** This attribute was added in IIS 7.5. There is no default value. |
