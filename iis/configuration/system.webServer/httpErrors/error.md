...
| `responseMode` | Optional enum attribute.<br><br>Specifies how custom error content is returned.<br><br>The **responseMode** attribute can be one of the following possible values. The default is `File`. <table> <tbody> <tr> <td>Value</td> <td>Description</td></tr> <tr> <td><code>File</code></td> <td>Serves static content, for example, a .html file for the custom error.<br><br>If <strong>responseMode</strong> is set to <strong>File</strong>, the path attribute value has to be a file path.<br><br>The numeric value is <code>0</code>. </td></tr> <tr> <td><code>ExecuteURL</code></td> <td>Serves dynamic content (for example, an .asp file) specified in the path attribute for the custom error.<br><br>If <strong>responseMode</strong> is set to <strong>ExecuteURL</strong>, the path value has to be a server relative URL.<br><br>The numeric value is <code>1</code>. </td></tr> <tr> <td><code>Redirect</code></td> <td>Redirects client browsers to a the URL specified in the path attribute that contains the custom error file.<br><br>If <strong>responseMode</strong> is set to <strong>Redirect</strong>, the path value has to be an absolute URL.<br><br>The numeric value is <code>2</code>.</td></tr></tbody></table> |
