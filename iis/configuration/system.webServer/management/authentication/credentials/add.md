...
| --- | --- |
| `name` | Required string attribute. Specifies the user name of the IIS Manager user account. |
| `password` | Required string attribute. Specifies the SHA256 hash of the user's password for the IIS Manager user account. This attribute is case sensitive. If you configure an IIS Manager user account by using IIS Manager, the password hash is computed automatically. If you want to calculate the hash of a password programmatically, you can use the System.Security.Cryptography.SHA256.ComputeHash method. For more information, see [SHA256 Class](https://go.microsoft.com/fwlink/?LinkId=96088) on MSDN. |
| `enabled` | Optional Boolean attribute. Specifies whether the IIS Manager user is enabled (**true**) or disabled (**false**). When true, the IIS Manager user can use IIS Manager to connect to all sites and applications for which they have authorized by a server administrator on the Web server. The default value is `true`. |
