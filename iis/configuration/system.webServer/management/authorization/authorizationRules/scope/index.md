...
| --- | --- |
| `path` | Required string attribute. Specifies the virtual path of a site or an application. IIS Manager users and Windows users can then be added to the child [&lt;add&gt;](https://www.iis.net/configreference/system.webserver/management/authorization/authorizationrules/scope/add) collection so they are authorized to connect to that site or application by using IIS Manager. |
