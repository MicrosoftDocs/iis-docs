---
title: "Trace Failed Requests &lt;traceFailedRequests&gt;"
...
ms.date: 09/26/2016
ms.assetid: dee94992-b0e8-4869-a1fe-0c07fda9b694
...
| Version | Notes |
| --- | --- |
| IIS 10.0 | The `<traceFailedRequests>` element was not modified in IIS 10.0. |
| IIS 8.5 | The `<traceFailedRequests>` element was not modified in IIS 8.5. |
| IIS 8.0 | The `<traceFailedRequests>` element was not modified in IIS 8.0. |
| IIS 7.5 | The `<traceFailedRequests>` element was not modified in IIS 7.5. |
| IIS 7.0 | The `<traceFailedRequests>` element was introduced in IIS 7.0. |
| IIS 6.0 | N/A |
...
4. On the **Server Roles** page, expand **Web Server (IIS)**, expand **Web Server**, expand **Health and Diagnostics**, and then select **Tracing**. Click **Next**.  
    [![Screenshot that shows a selected checkbox for Tracing, under the Health and Diagnostics node.](index/_static/image2.png)](index/_static/image1.png) .
...
3. Expand **Internet Information Services**, expand **World Wide Web Services**, expand **Health and Diagnostics**, and then select **Tracing**.  
    [![Screenshot that shows a selected checkbox for Tracing in a Windows 8 operating system.](index/_static/image4.png)](index/_static/image3.png)- Click **OK**.
4. Click **Close**.
...
4. On the **Select Role Services** page of the **Add Role Services Wizard**, select **Tracing**, and then click **Next**.  
    [![Screenshot that shows a selected checkbox for Tracing in a Windows Server 2008 operating system.](index/_static/image6.png)](index/_static/image5.png)
...
4. Select **Tracing**, and then click **OK**.  
    [![Screenshot that shows a selected checkbox for Tracing in a Windows Vista or Windows 7 operating system.](index/_static/image8.png)](index/_static/image7.png)
...
3. In the **Actions** pane, click **Failed Request Tracing...**  
    [![Screenshot that shows the Default Web Site Home Pane.](index/_static/image10.png)](index/_static/image9.png)
4. In the **Edit Web Site Failed Request Tracing Settings** dialog box, select the **Enable** check box to enable tracing, leave the default value or type a new directory where you want to store failed request log files in the **Directory** box, type the number of failed request trace files you want to store in the **Maximum number of trace files** box, and then click **OK**.  
    [![Screenshot that shows the Edit Web Site Failed Request Tracing Settings dialog box. Enable has a selected checkbox.](index/_static/image12.png)](index/_static/image11.png)
...
3. In the **Home** pane, double-click **Failed Request Tracing Rules**.  
    [![Screenshot that shows the Default Web Site Home Pane. Failed request Tracing Rule is selected.](index/_static/image14.png)](index/_static/image13.png)
...
5. On the **Specify Content to Trace** page of the **Add Failed Request Tracing Rule** Wizard, select the content type you want to trace, and then click **Next**.  
    [![Screenshot that shows All content selected.](index/_static/image16.png)](index/_static/image15.png)
6. On the **Define Trace Conditions** page, select the conditions you want to trace, and then click **Next**. Trace conditions can include any combination of status codes, a time limit that a request should take, or the event severity. If you specify all conditions, the first condition that is met generates the failed request trace log file.  
    [![Screenshot that shows Status code selected on the Define Trace Conditions page.](index/_static/image18.png)](index/_static/image17.png)
7. On the **Select Trace Providers** page, select one or more of the trace providers under **Providers**.  
    [![Screenshot that shows the Select Trace Providers page for Providers.](index/_static/image19.png)](index/_static/image19.png)
8. On the **Select Trace Providers** page, select one or more of the verbosity levels under **Verbosity**.  
    [![Screenshot that shows the Select Trace Providers page for Verbosity.](index/_static/image22.png)](index/_static/image21.png)
