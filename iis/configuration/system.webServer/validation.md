...
- Your application defines an `<httpModules>` section in its Web.config file. 
    ...
    In IIS 7 Integrated mode, ASP.NET modules are specified with native modules in a unified [&lt;modules&gt;](https://www.iis.net/configreference/system.webserver/modules) section under [&lt;system.webServer&gt;](https://www.iis.net/configreference/system.webserver).
- Your application defines an `<httpHandlers>` section in its Web.config file. 

    In IIS 7 Integrated mode, the ASP.NET handler mappings are specified in a unified [&lt;handlers&gt;](https://www.iis.net/configreference/system.webserver/handlers) section inside [&lt;system.webServer&gt;](https://www.iis.net/configreference/system.webserver). The [&lt;handlers&gt;](https://www.iis.net/configreference/system.webserver/handlers)< section="" replaces="" both="" the="" asp.net="">`<httpHandlers>` and IIS script-processor-mapping configurations, which were both required to set up an ASP.NET 1.0 handler mapping.
