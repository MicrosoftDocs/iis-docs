...
| `allowCustomProperties` | Optional **Boolean** attribute. **true** if custom WebDAV properties are allowed; otherwise, **false**. > [!NOTE]
 > Custom WebDAV properties are also known as "dead" properties. > [!NOTE]
 > If `allowCustomProperties` is set to **true** but no property stores are defined or there are no XML namespace to property store mappings, no custom properties can be stored. The default value is `true`. |
