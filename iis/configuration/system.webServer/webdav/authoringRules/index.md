...
The element contains a series of [`<add>`](../../system.webserver/webdav/authoringrules/add.md) elements that define the individual authoring rules, which contain the access permissions for users or groups for specific content types.

WebDAV authoring rules may seem somewhat analogous to authorization settings that are found in the [`<authorization>`](../../system.webserver/security/authorization.md) element, but are stored separately so that IIS maintains two different sets of authorization: one for regular HTTP (non-WebDAV) requests and the other for WebDAV requests.
...
| Element | Description |
| --- | --- |
| [`add`](../../system.webserver/webdav/authoringrules/add.md) | Optional element. Adds an authoring rule to the collection of authoring rules. |
