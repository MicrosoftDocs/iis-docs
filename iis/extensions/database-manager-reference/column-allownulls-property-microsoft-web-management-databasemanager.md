...
description: This article contains information on syntax and permissions for the Column.AllowNulls property. There are also linkes to more resources.
...
f1_keywords:
- Microsoft.Web.Management.DatabaseManager.Column.AllowNulls
- Microsoft.Web.Management.DatabaseManager.Column.get_AllowNulls
- Microsoft.Web.Management.DatabaseManager.Column.set_AllowNulls
dev_langs:
- csharp
- jscript
- vb
...
# Column.AllowNulls Property

Gets or sets a value that indicates whether the database column allows nulla null reference (Nothing in Visual Basic) values.
...
```vb
'Declaration
Public Overridable Property AllowNulls As Boolean
'Usage
...
```cpp
public:
virtual property bool AllowNulls {
    bool get ();
    void set (bool value);
}
```
...
true if the column allows nulla null reference (Nothing in Visual Basic) values; otherwise, false.  

## Examples

The following code sample shows how to use the [GetTableInfo](idbtablemanager-gettableinfo-method-microsoft-web-management-databasemanager.md) method to retrieve the table information for the table name that the database manager provides by using an OLEDB connection.
