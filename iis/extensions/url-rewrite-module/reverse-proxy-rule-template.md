...
author: ruslany
description: Learn how to use the "Reverse Proxy" rule template in IIS URL Rewrite Module to create inbound and outbound rewrite rules for efficient HTTP request routing.
ms.date: 03/11/2010
...
Blocking or normalizing such patterns before the ARR module handles them helps prevent exploitation.
...
If you want to strip path parameters and re-normalize the URL so all URL Rewrite rules apply to the normalized URL, you need a redirect round trip. For example:
Input:
`/abc/..;boo/xyz/`
Redirected to:
