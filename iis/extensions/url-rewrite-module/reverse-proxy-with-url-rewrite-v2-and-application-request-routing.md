...
5. Enter `^/(webmail|payroll)/.*` in the **Pattern** textbox. This regular expression is used to match the URL paths that start with either `/webmail` or `/payroll`. The parenthesis within the pattern captures the part of the matched URL string, so that it can be used when constructing the replacement URL.
