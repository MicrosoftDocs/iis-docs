...
2. Once the IIS Manager starts up, expand the server node in the left-hand window so that both the **Application Pools** node and **Sites** node are showing. When you do this the IIS Manager will popup a dialog asking if you want to stay connected with the latest Web Platform Components. For this walkthrough, you can click **No**. At this point the IIS Manager looks like the following picture:  
