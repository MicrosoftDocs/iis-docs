...
``` vb
'Declaration
PublicPropertyTimerIntervalAsTimeSpan
'Usage
DiminstanceAsAdvancedLoggingPluginDimvalueAsTimeSpanvalue = instance.TimerInterval

instance.TimerInterval = value
```

``` csharp
publicTimeSpanTimerInterval { get; set; }
```

``` c++
public:
propertyTimeSpanTimerInterval {
    TimeSpanget ();
...
``` jscript
function getTimerInterval () : TimeSpanfunction setTimerInterval (value : TimeSpan)
```
