...
``` vb
'Declaration
PublicPropertyResponseAsStream
'Usage
DiminstanceAsCacheResponseDimvalueAsStreamvalue = instance.Response
```

``` csharp
publicStreamResponse { get; privateset; }
```

``` c++
public:
propertyStream^ Response {
    Stream^ get ();
...
``` jscript
function getResponse () : Streamprivatefunction setResponse (value : Stream)
```
