...
``` vb
'Declaration
PublicPropertyClipUriAsUri
'Usage
DiminstanceAsClipInformationDimvalueAsUrivalue = instance.ClipUri
```

``` csharp
publicUriClipUri { get; privateset; }
```
...
``` jscript
function getClipUri () : Uriprivatefunction setClipUri (value : Uri)
```
