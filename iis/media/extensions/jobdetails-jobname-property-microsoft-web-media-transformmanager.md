...
PublicPropertyJobNameAsStringGetSet
'Usage
DiminstanceAsJobDetailsDimvalueAsStringvalue = instance.JobName
...
``` csharp
[DataMemberAttribute]
publicstringJobName { get; set; }
```

``` c++
[DataMemberAttribute]
public:
propertyString^ JobName {
    String^ get ();
...
``` fsharp
[<DataMemberAttribute>]
memberJobName : stringwithget, set
```

``` jscript
function getJobName () : Stringfunction setJobName (value : String)
```
