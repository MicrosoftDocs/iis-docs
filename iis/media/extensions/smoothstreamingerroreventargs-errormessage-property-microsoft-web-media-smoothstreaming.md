...
``` vb
'Declaration
PublicPropertyErrorMessageAsString
'Usage
DiminstanceAsSmoothStreamingErrorEventArgsDimvalueAsStringvalue = instance.ErrorMessage

instance.ErrorMessage = value
```

``` csharp
publicstringErrorMessage { get; set; }
```

``` c++
public:
propertyString^ ErrorMessage {
    String^ get ();
...
``` jscript
function getErrorMessage () : Stringfunction setErrorMessage (value : String)
```
