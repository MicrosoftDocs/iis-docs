...
``` vb
'Declaration
PublicPropertyAudioStreamCountAsInteger
'Usage
DiminstanceAsSmoothStreamingMediaElementDimvalueAsIntegervalue = instance.AudioStreamCount

instance.AudioStreamCount = value
```

``` csharp
publicintAudioStreamCount { get; set; }
```

``` c++
public:
propertyintAudioStreamCount {
    intget ();
    voidset (intvalue);
}
```
...
``` jscript
function getAudioStreamCount () : intfunction setAudioStreamCount (value : int)
```
