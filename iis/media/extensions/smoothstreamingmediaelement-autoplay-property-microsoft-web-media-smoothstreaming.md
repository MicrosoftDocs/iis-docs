...
``` vb
'Declaration
PublicPropertyAutoPlayAsBoolean
'Usage
DiminstanceAsSmoothStreamingMediaElementDimvalueAsBooleanvalue = instance.AutoPlay

instance.AutoPlay = value
```

``` csharp
publicboolAutoPlay { get; set; }
```

``` c++
public:
propertyboolAutoPlay {
    boolget ();
    voidset (boolvalue);
}
```
...
``` jscript
function getAutoPlay () : booleanfunction setAutoPlay (value : boolean)
```
