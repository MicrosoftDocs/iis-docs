...
``` vb
'Declaration
PublicPropertyBalanceAsDouble
'Usage
DiminstanceAsSmoothStreamingMediaElementDimvalueAsDoublevalue = instance.Balance

instance.Balance = value
```

``` csharp
publicdoubleBalance { get; set; }
```

``` c++
public:
propertydoubleBalance {
    doubleget ();
    voidset (doublevalue);
}
```
...
``` jscript
function getBalance () : doublefunction setBalance (value : double)
```
