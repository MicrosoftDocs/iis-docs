...
``` vb
'Declaration
PublicSharedReadOnlyCanPausePropertyAsDependencyProperty
'Usage
DimvalueAsDependencyPropertyvalue = SmoothStreamingMediaElement.CanPauseProperty
```

``` csharp
publicstaticreadonlyDependencyPropertyCanPauseProperty
```

``` c++
public:
staticinitonlyDependencyProperty^ CanPauseProperty
```

``` jscript
publicstaticfinalvarCanPauseProperty : DependencyProperty
```
