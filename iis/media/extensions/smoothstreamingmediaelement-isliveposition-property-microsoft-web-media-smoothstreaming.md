...
``` vb
'Declaration
PublicReadOnlyPropertyIsLivePositionAsBoolean
'Usage
DiminstanceAsSmoothStreamingMediaElementDimvalueAsBooleanvalue = instance.IsLivePosition
```

``` csharp
publicboolIsLivePosition { get; }
```

``` c++
public:
propertyboolIsLivePosition {
    boolget ();
}
```

``` jscript
function getIsLivePosition () : boolean
```
