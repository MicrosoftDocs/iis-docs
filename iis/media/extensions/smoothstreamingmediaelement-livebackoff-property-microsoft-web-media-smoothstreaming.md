...
``` vb
'Declaration
PublicPropertyLiveBackOffAsTimeSpan
'Usage
DiminstanceAsSmoothStreamingMediaElementDimvalueAsTimeSpanvalue = instance.LiveBackOff

instance.LiveBackOff = value
```

``` csharp
publicTimeSpanLiveBackOff { get; set; }
```

``` c++
public:
propertyTimeSpanLiveBackOff {
    TimeSpanget ();
...
``` jscript
function getLiveBackOff () : TimeSpanfunction setLiveBackOff (value : TimeSpan)
```
