...
``` vb
'Declaration
PublicReadOnlyPropertyManifestInfoAsManifestInfo
'Usage
DiminstanceAsSmoothStreamingMediaElementDimvalueAsManifestInfovalue = instance.ManifestInfo
```

``` csharp
publicManifestInfoManifestInfo { get; }
```

``` c++
public:
propertyManifestInfo^ ManifestInfo {
    ManifestInfo^ get ();
}
```

``` jscript
function getManifestInfo () : ManifestInfo
```
