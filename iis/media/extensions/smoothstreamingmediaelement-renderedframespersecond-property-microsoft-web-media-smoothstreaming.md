...
``` vb
'Declaration
PublicReadOnlyPropertyRenderedFramesPerSecondAsDouble
'Usage
DiminstanceAsSmoothStreamingMediaElementDimvalueAsDoublevalue = instance.RenderedFramesPerSecond
```

``` csharp
publicdoubleRenderedFramesPerSecond { get; }
```

``` c++
public:
propertydoubleRenderedFramesPerSecond {
    doubleget ();
}
```

``` jscript
function getRenderedFramesPerSecond () : double
```
