...
``` vb
'Declaration
PublicReadOnlyPropertyChunkListAsIList(OfChunkInfo)
'Usage
DiminstanceAsStreamInfoDimvalueAsIList(OfChunkInfo)

value = instance.ChunkList
```

``` csharp
publicIList<ChunkInfo> ChunkList { get; }
```

``` c++
public:
propertyIList<ChunkInfo^>^ ChunkList {
    IList<ChunkInfo^>^ get ();
}
```

``` jscript
function getChunkList () : IList<ChunkInfo>
```
