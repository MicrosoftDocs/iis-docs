...
``` vb
'Declaration
PublicSharedSubDisableTraceArea ( _
    traceAreaAsTraceArea _
)
'Usage
DimtraceAreaAsTraceAreaTracing.DisableTraceArea(traceArea)
```

``` csharp
publicstaticvoidDisableTraceArea(
    TraceAreatraceArea
)
```
...
``` c++
public:
staticvoidDisableTraceArea(
    TraceAreatraceArea
)
```

``` jscript
publicstaticfunctionDisableTraceArea(
    traceArea : TraceArea
)
```
