...
``` vb
'Declaration
PublicSharedSubDisableTraceDestination ( _
    destinationAsTraceDestination _
)
'Usage
DimdestinationAsTraceDestinationTracing.DisableTraceDestination(destination)
```

``` csharp
publicstaticvoidDisableTraceDestination(
    TraceDestinationdestination
)
```

``` c++
public:
staticvoidDisableTraceDestination(
    TraceDestinationdestination
)
```

``` jscript
publicstaticfunctionDisableTraceDestination(
