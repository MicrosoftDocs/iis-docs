...
``` vb
'Declaration
PublicSharedSubDisableTraceLevel ( _
    traceLevelAsTraceLevel _
)
'Usage
DimtraceLevelAsTraceLevelTracing.DisableTraceLevel(traceLevel)
```

``` csharp
publicstaticvoidDisableTraceLevel(
...
``` c++
public:
staticvoidDisableTraceLevel(
    TraceLeveltraceLevel
)
```

``` jscript
publicstaticfunctionDisableTraceLevel(
