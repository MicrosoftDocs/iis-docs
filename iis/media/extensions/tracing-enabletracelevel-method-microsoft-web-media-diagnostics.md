...
``` vb
'Declaration
PublicSharedSubEnableTraceLevel ( _
    traceLevelAsTraceLevel _
)
'Usage
DimtraceLevelAsTraceLevelTracing.EnableTraceLevel(traceLevel)
```

``` csharp
publicstaticvoidEnableTraceLevel(
...
``` c++
public:
staticvoidEnableTraceLevel(
    TraceLeveltraceLevel
)
```

``` jscript
publicstaticfunctionEnableTraceLevel(
