...
``` vb
'Declaration
PublicSharedFunctionIsTraceAreaEnabled ( _
    traceAreaAsTraceArea _
) AsBoolean
'Usage
DimtraceAreaAsTraceAreaDimreturnValueAsBooleanreturnValue = Tracing.IsTraceAreaEnabled(traceArea)
```

``` csharp
publicstaticboolIsTraceAreaEnabled(
    TraceAreatraceArea
)
```
...
``` c++
public:
staticboolIsTraceAreaEnabled(
    TraceAreatraceArea
)
```

``` jscript
publicstaticfunctionIsTraceAreaEnabled(
    traceArea : TraceArea
) : boolean
```
