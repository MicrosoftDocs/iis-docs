...
#### Parameters  
 `pProvider`  
 [IN] A pointer to an [IGlobalStopListeningProvider](../../web-development-reference\webdev-native-api-reference/iglobalstoplisteningprovider-interface.md) interface.  
...
## Remarks  
 When a global module has registered for the [GL_STOP_LISTENING](../../web-development-reference\webdev-native-api-reference/request-processing-constants.md) event notification, IIS will call the module's `OnGlobalStopListening` method when IIS stops accepting new requests for an application.  
  
> [!NOTE]
>  Global modules can register for the `GlobalStopListening` event notification by registering for `GL_STOP_LISTENING` in the module's [RegisterModule](../../web-development-reference\webdev-native-api-reference/pfn-registermodule-function.md) function.  
