...
Retrieves the attributes of the corresponding file for an [IHttpFileInfo](../../web-development-reference\webdev-native-api-reference/ihttpfileinfo-interface.md) interface.  
...
## Remarks  
 The `GetAttributes` method retrieves a `DWORD` that contains a bitmask of attribute values for the file that is currently referenced by an `IHttpFileInfo` interface. For example, if you write an application that calls the [IHttpContext::GetFileInfo](../../web-development-reference\webdev-native-api-reference/ihttpcontext-getfileinfo-method.md) method to retrieve an `IHttpFileInfo` interface, the `GetAttributes` method will return attributes of the file that is being used for the current request.  
