...
## Remarks  
 The `GetForwardedUrl` method retrieves the URL after a call to the [IHttpRequest::SetUrl](../../../webdevelopment-reference\native-code-api\webdev-native-api-reference/ihttprequest-seturl-method.md) method, but only when the URL does not refer to a local address. If you use `SetUrl` to specify a local URL, the `GetForwardedUrl` method will return NULL.  
