...
## Remarks  
 Instances of this class are contained in the `Forms` property of the [AuthenticationSection](../wmi-provider/authenticationsection-class1.md) class.  
...
|3|`UseDeviceProfile`|Specifies that [!INCLUDE[vstecasp](../wmi-provider/includes/vstecasp-md.md)] determines whether to use cookies based on the `Cookies` property of the <xref:System.Web.Configuration.HttpCapabilitiesBase> class. If the setting indicates that the browser or device supports cookies, the requesting feature uses cookies; otherwise, the requesting feature uses an identifier in the query string.|  
